...
async def mute_user(user, guild, time_diff):
    ...
    try:
        ...
        await muted_channel.send(f"You have been muted for {MUTE_DURATION_MINUTES} minutes for spamming messages across multiple channels.")
        ...
        log_channel = client.get_channel(log_chat_id)
        if log_channel:
            await log_channel.send(f"❌ Failed to mute {user.mention} - Missing permissions!")
